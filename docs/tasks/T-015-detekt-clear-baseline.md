...
- 2026-07-13: **Wave 7 — `@file:Suppress("UnsafeCallOnNullableType")` на 5 файлах** (13 findings). Kotlin+JPA идиома: entity-поля объявлены `var …: X? = null` (JPA требует), но при чтении из БД гарантированно non-null (domain invariant). `!!` в местах доступа — практика для этой конкретной пары. Файлы: `StatsService` (8 `!!`), `AdminGameplayService`, `AuthService`, `CoreGameplayService`, `ShuffleStrategy`. `?: error(...)` в каждом месте просто дублировал бы Kotlin null-check без ценности. Итого **36→23 findings**. `./gradlew check` зелёный.
- 2026-07-13: **Wave 8 — real code fixes** (9 findings). Исправления в коде, не в baseline:
  - `ShuffleStrategy`: `throw IllegalStateException(msg)` × 3 → `check(...) { msg }` (UseCheckOrError).
  - `JwtAuthenticationFilter`: `authHeader == null || authHeader == ""` → `authHeader.isNullOrEmpty()` (UseIsNullOrEmpty).
  - `RestAccessDeniedHandler`: `request?.servletPath ?: ""` → `request?.servletPath.orEmpty()` (UseOrEmpty).
  - `AuthService`: убран лишний `!!` — `authenticateUserRequest.id!!` → `.id` (тип `UUID`, non-null; UnnecessaryNotNullOperator).
  - `AdminGameplayService`: удалены `val sessionId = requireNotNull(session.id)` в `abortSession`/`startNextRound` — параметр `sessionId: UUID` уже non-null и совпадает с `session.id` после `getSessionEntity` (NoNameShadowing).
