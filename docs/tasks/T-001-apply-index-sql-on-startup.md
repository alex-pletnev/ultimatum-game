...
- 2026-07-16: **поглощено T-044**. `IndexSqlInitializer.kt`, `IndexSqlInitializerTest.kt`, `src/main/resources/index.sql` удалены — их содержимое (pg_trgm extension + idx_session_name_trgm + ix_npc_profile_user_id) теперь в `V1__baseline.sql` (Flyway). Задача остаётся `done`, но код физически заменён.
