...
### Хостинг + smoke-test
- [x] Выбран и обоснован хостинг: Yandex.Cloud (Compute VM + Managed PG + Container Registry + Lockbox + Caddy для HTTPS через nip.io). Изначально был Serverless Container — не подошёл из-за cold-start Spring Boot ~40s (см. Лог 2026-07-17).
- [x] Задеплоено. `https://158-160-48-113.nip.io/api/v1`. Secrets в Lockbox, не в git.
- [x] Прогнан smoke-test: `/actuator/health` 200 UP, `/auth/quick-register` → 201 с JWT, `/session` с JWT → 200.
...
- 2026-07-17: **Pivot внутри YC — Serverless Container → Compute VM**. Serverless
  Container не подошёл: Spring Boot cold-start ~40s (12s Spring + 29s Hikari PG
  connect) > YC execution-timeout 30s. YC пересоздавал контейнер в цикле.
  min_instances=1 не помог — YC перезапускал inactive instance.
  Переехали на Compute VM Ubuntu 24.04 (s3-c2-m2, 2vCPU 20% shared, 2GB RAM, 20GB HDD,
  ~250-350₽/мес) + static public IP + Caddy для HTTPS через nip.io (auto-cert
  Let's Encrypt на домене `158-160-48-113.nip.io`).
  Cloud-init автоматизирует: apt install docker.io + caddy → docker login через SA
  metadata → docker pull image из cr.yandex → secrets из Lockbox через REST API →
  docker run с env-vars → Caddy reverse-proxy на :8080.
  6 итераций fix'ов cloud-init'а (dpkg TTY-issues, --ssh-key vs metadata conflict,
  SA-token retry, YAML `content:` substitution съедающий `$var` → base64-embed).
  Root cause последнего — cloud-init YAML `content: |` парсит `${var}` references и
  превращает undefined в empty. Encoding: b64 обходит.
  Дополнительно: Flyway CREATE EXTENSION pg_trgm требует superuser rights, YC MDB
  не даёт → `yc managed-postgresql database update ultimatum --extensions pg_trgm`
  как admin-op (интегрировано в deploy-yc-vm.sh Step 2).
- 2026-07-17: **Backend доступен**: `https://158-160-48-113.nip.io/api/v1`.
