...
- [ ] Query params `state`, `sessionType`, `openToConnect` — все optional, комбинируются AND.
- [ ] Repository: `Specification`-based или дополнительный `@Query`. Использовать `@EntityGraph`/fetch join там где нужно.
- [ ] Тесты — фильтры по каждому + комбинация.
- [ ] Обновить `docs/05-rest-api.md`, regenerate `openapi.json`.
